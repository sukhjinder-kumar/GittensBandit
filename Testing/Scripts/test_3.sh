...
# QLearning
echo "Starting QLearning test3 with epsilon-greedy-gittin temperature mode"
start_time1=$(date +%s)  # Record start time

sleep 10
python3 -m Scripts.main QLearning test3 --num_runs=${num_runs} --num_epochs=${num_epochs} \
...
echo "Starting Reinforce test3 with linear schedule"
start_time2=$(date +%s)  # Record start time

sleep 10
python3 -m Scripts.main Reinforce test3 --num_runs=${num_runs} --num_epochs=${num_epochs} \
...
echo "Starting Reinforce test3 with none schedule"
...
echo "Finished QLearning test3 with epsilon-greedy-gittin temperature mode in $duration1_formatted"
...
echo "Finished Reinforce test3 with linear schedule in $duration2_formatted"
...
echo "Finished Reinforce test3 with none schedule in $duration3_formatted"
