...
sleep 10
python3 -m Scripts.main QLearning test2 --num_runs=${num_runs} --num_epochs=${num_epochs} \
...
sleep 10
python3 -m Scripts.main Reinforce test2 --num_runs=${num_runs} --num_epochs=${num_epochs} \
...
echo "Finished QLearning test2 with epsilon-greedy-gittin temperature mode in $duration1_formatted"
...
echo "Finished Reinforce test2 with linear schedule in $duration2_formatted"
...
echo "Finished Reinforce test2 with none schedule in $duration3_formatted"
